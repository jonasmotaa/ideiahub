...
// Consulta SQL
$sql = "SELECT SUM(valor) AS soma_valores
        FROM transacao 
...
$sql = "SELECT id, email, dat, valor, descricao, cliente, tipo FROM transacao WHERE MONTH(dat) = MONTH(CURDATE()) AND YEAR(dat) = YEAR(CURDATE())";;
$transacoes = $mysqli->query($sql);
        ...
        <div class="row">
          <div class="col-12 col-lg-16 d-flex">
            <div class="card w-100">
              <div class="card-body">
                <h5 class="mb-3 fw-bold">Transações do mês</h5>
                    ...
                    <table class="table align-middle">
                      ...
                      <tbody>
                        <?php if ($transacoes->num_rows > 0) {
                          while ($rowtransacoes = $transacoes->fetch_assoc()) {   ?>
                            <tr <?php if ($rowtransacoes['tipo'] == 1) {
                                  echo "style='background-color: red'";
                                } ?>>
                              <td>
                                <div class="d-flex align-items-center gap-3">
                                  <div class="product-info">
                                    <p class="product-title"><?php echo $rowtransacoes['cliente'] ?></p>
                                  </div>
                                </div>
                              </td>

                              <td><?php echo date('d/m/Y', strtotime($rowtransacoes['dat'])) ?></td>
                              <td><?php echo "R$ " . number_format($rowtransacoes['valor'], 2, ',', '.') ?></td>
                              <td><?php echo $rowtransacoes['descricao'] ?></td>

                              <?php $sql = "SELECT u.nome 
                                            FROM usuario u
                                            INNER JOIN transacao t ON u.email = t.email
                                            WHERE t.email = ?";

                              // Usar uma consulta preparada para evitar injeção de SQL
                              $stmt = $mysqli->prepare($sql);
                              $stmt->bind_param("s", $rowtransacoes['email']);
                              $stmt->execute();
